/**
 * @file preintegration.hpp
 * @brief IMU preintegration for VoxelSLAM system
 * 
 * Implements IMU preintegration algorithms for efficient processing
 * of IMU measurements between LiDAR frames. Handles bias estimation,
 * uncertainty propagation, and coordinate frame transformations.
 */
...
double imupre_scale_gravity = 1.0;        ///< Gravity scaling factor for IMU measurements
Eigen::Matrix<double, 6, 6> noiseMeas, noiseWalk;  ///< IMU noise covariance matrices

/**
 * @class IMU_PRE
 * @brief IMU preintegration class for efficient IMU processing
 * 
 * Preintegrates IMU measurements between consecutive LiDAR frames
 * to reduce computational cost and improve accuracy. Handles bias
...
public:
  ...
  Eigen::Matrix3d R_delta;          ///< Relative rotation from preintegration
  Eigen::Vector3d p_delta, v_delta; ///< Relative position and velocity
  Eigen::Vector3d bg, ba;           ///< Gyroscope and accelerometer biases
