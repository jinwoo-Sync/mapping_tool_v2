 ...
 * @brief Loop closure detection and pose graph optimization for VoxelSLAM
 * 
 * Implements loop closure detection algorithms, ICP-based refinement,
 * and global pose graph optimization using GTSAM. Provides robust
 * loop detection and correction for drift reduction in SLAM.
 */
...
/**
 * @struct ScanPose
 * @brief Structure containing pose and point cloud data for a scan
 * 
 * Stores the pose information and associated point cloud for
 * loop closure detection and pose graph optimization.
 */
struct ScanPose
{
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  IMUST x;                         ///< IMU state (pose, velocity, biases)
  PVecPtr pvec;                    ///< Associated point cloud vector
  Eigen::Matrix<double, 6, 1> v6;  ///< 6-DOF pose vector
