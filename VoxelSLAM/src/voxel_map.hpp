/**
 * @file voxel_map.hpp
 * @brief Voxel-based map representation and management for VoxelSLAM
 * 
 * Implements efficient voxel-based map data structures and algorithms
 * for real-time LiDAR SLAM. Provides voxel grid management, point
 * association, and spatial indexing for fast nearest neighbor queries.
 */
...
struct pointVar 
{
  ...
  Eigen::Vector3d pnt;
  Eigen::Matrix3d var;
  float intensity = 0.0; // Store original intensity information
};
...
void down_sampling_pvec(PVec &pvec, double voxel_size, pcl::PointCloud<PointType> &pl_keep)
{
  ...
  for(pointVar &pv: pvec)
  {
    ...
    else
    {
      ...
      pp.first.intensity = (pp.first.intensity * pp.second + pv.intensity) / (pp.second + 1);
      pp.second += 1;
    }
  }
  ...
  for(auto iter=feat_map.begin(); iter!=feat_map.end(); ++iter)
  {
    ...
    ap.intensity = pv.intensity; // Use original intensity
...
public:
  ...
  void tras_display(int win_count, pcl::PointCloud<PointType> &pl_fixd, pcl::PointCloud<PointType> &pl_wind, vector<IMUST> &x_buf)
  {
    if(octo_state == 0)
    {
      ...
      PointType ap; 
      ap.intensity = 0; // Default intensity value
