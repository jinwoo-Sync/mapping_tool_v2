/**
 * @file ekf_imu.hpp
 * @brief Extended Kalman Filter for IMU state estimation
 * 
 * Implements EKF-based IMU integration and state propagation for
 * LiDAR-inertial SLAM system. Handles IMU initialization, motion
 * compensation, and uncertainty propagation.
 */
...
/**
 * @class IMUEKF
 * @brief Extended Kalman Filter for IMU state estimation and integration
 * 
 * Provides IMU initialization, state propagation, motion compensation,
 * and uncertainty estimation for LiDAR-inertial SLAM. Handles sensor
 * fusion and temporal synchronization between IMU and LiDAR measurements.
 */
class IMUEKF
{
public:
  ...
  // Public member to store motion compensation data for external logging
  vector<pair<double, double>> motion_comp_data;
  
  // EKF-specific timestamp logging variables
  int ekf_frame_counter = 0;
  bool enable_ekf_timestamp_logging = true;
  string ekf_timestamp_log_dir = "/media/yjcho/MT_009/data/voxel_slam/ekf_timestamp";

  // Function to create directory if it doesn't exist
  void create_ekf_directory_if_not_exists(const string& dir_path) {
      struct stat st = {0};
  ...
  // Function to save LiDAR point timestamps in EKF process
  void save_ekf_lidar_timestamps(const pcl::PointCloud<PointType> &pcl_in, int frame_id) {
      if (!enable_ekf_timestamp_logging) return;
      
      create_ekf_directory_if_not_exists(ekf_timestamp_log_dir);
      
      string filename = ekf_timestamp_log_dir + "/ekf_lidar_timestamps_frame_" + to_string(frame_id) + ".txt";
      ofstream file(filename);
      
      if (file.is_open()) {
          file << fixed << setprecision(9);
          file << "# EKF LiDAR Point Timestamps for Frame " << frame_id << endl;
          file << "# Format: point_index timestamp_seconds" << endl;
          file << "# Total points: " << pcl_in.size() << endl;
          
          for (size_t i = 0; i < pcl_in.size(); i++) {
              file << i << " " << pcl_in.points[i].curvature << endl;
          }
          
          file.close();
          printf("EKF LiDAR timestamps saved to: %s (%zu points)\n", filename.c_str(), pcl_in.size());
      } else {
  ...
  void motion_blur(IMUST &xc, pcl::PointCloud<PointType> &pcl_in, deque<sensor_msgs::Imu::Ptr> &imus)
  {
    ...
    // Clear previous motion compensation data
    motion_comp_data.clear();
    ...
    for(auto it_imu=imus.begin(); it_imu!=imus.end()-1; it_imu++)
    {
      ...
      // std::cout << "last_pcl_end_time: " << last_pcl_end_time << std::endl;
      // std::cout << "tail.header.stamp.toSec(): " << tail.header.stamp.toSec() << std::endl;
      // std::cout << "head.header.stamp.toSec(): " << head.header.stamp.toSec() << std::endl;
    ...
    for(int i=imu_poses.size()-1; i>=0; i--)
    {
      ...
      for(; it_pcl->curvature > head.t; it_pcl--)
      {
        ...
        // Store dt and head.t for external logging
        motion_comp_data.push_back(make_pair(dt, head.t));
  ...
  int process(IMUST &x_curr, pcl::PointCloud<PointType> &pcl_in, deque<sensor_msgs::Imu::Ptr> &imus)
  {
    ...
    // Save EKF LiDAR timestamps before motion compensation
    ekf_frame_counter++;
    save_ekf_lidar_timestamps(pcl_in, ekf_frame_counter);
