/**
 * @file voxelslam.cpp
 * @brief VoxelSLAM main implementation file
 * 
 * Contains the core VoxelSLAM algorithm implementation including
 * initialization, odometry estimation, loop closure detection,
 * and global optimization for LiDAR-inertial SLAM.
 */
...
// Global flag for gravity compensated IMU mode
// Global variables for original resolution saving
bool is_save_original_resolution = false;
bool use_full_resolution_odometry = false;
...
public:
  ...
  /**
   * @brief Publishes odometry information for visualization
   * @param xc Current IMU state containing pose and velocity
   */
  ...
  void pub_localtraj(PLV(3) &pwld, double jour, IMUST &x_curr, int cur_session, pcl::PointCloud<PointType> &pcl_path)
  {
    ...
    for(Eigen::Vector3d &pw: pwld)
    {
      ...
      ap.y = pvec.y();
      ap.z = pvec.z();
      ap.intensity = 50; // Default visualization intensity
  ...
  // Overloaded function - with intensity data
  void pub_localtraj(PLV(3) &pwld, vector<float> &intensity_buf, double jour, IMUST &x_curr, int cur_session, pcl::PointCloud<PointType> &pcl_path)
  {
    pub_odom_func(x_curr);
    pcl::PointCloud<PointType> pcl_send;
    pcl_send.reserve(pwld.size());
    for(size_t i = 0; i < pwld.size(); i++)
    {
      ...
      ap.y = pvec.y();
      ap.z = pvec.z();
      ap.intensity = intensity_buf[i]; // Use actual sensor intensity data
      pcl_send.push_back(ap);
    }
    pub_pl_func(pcl_send, pub_scan);
  ...
  /**
   * @brief Publishes local map for visualization
   * @param mgsize Map generation size
   * @param cur_session Current session ID
   * @param pvec_buf Buffer of point vector data
   * @param x_buf Buffer of IMU states
  ...
  void pub_localmap(int mgsize, int cur_session, vector<PVecPtr> &pvec_buf, vector<IMUST> &x_buf, pcl::PointCloud<PointType> &pcl_path, int win_base, int win_count)
  {
    ...
    for(int i=0; i<mgsize; i++)
    {
      for(int j=0; j<pvec_buf[i]->size(); j+=3)
      {
        ...
        ap.y = pvec[1];
        ap.z = pvec[2];
        ap.intensity = pv.intensity; // Use actual sensor intensity data instead of session ID
        pcl_send.push_back(ap);
      }
    }
    ...
    for(int id: ids)
    {
      ...
      for(int i=0; i<smps.size(); i++)
      {
        ...
        // for(int j=0; j<smps[i]->plptr->size(); j+=1)
        {
          ...
          pp.intensity = ap.intensity; // Use actual point intensity
          pp.curvature = ap.curvature;
          pp.normal_x = ap.normal_x; 
          pp.normal_y = ap.normal_y; 
          pp.normal_z = ap.normal_z;
          
          // Strong Debug: Always print first 10 intensity values and every 1000th
          static int debug_intensity_count = 0;
          static float min_intensity = 999.0f, max_intensity = -999.0f;
          debug_intensity_count++;
          if(ap.intensity < min_intensity) min_intensity = ap.intensity;
          if(ap.intensity > max_intensity) max_intensity = ap.intensity;
...
public:
  ...
  void save_pcd(PVecPtr pptr, IMUST &xx, int count, const string &savename)
  {
    ...
    for(pointVar &pw: *pptr)
    {
      ...
      ap.intensity = pw.intensity; // Save intensity information to PCD file
  ...
  void save_original_resolution_pcd(pcl::PointCloud<PointType>::Ptr pl_orig, IMUST &xx, int count, const string &savename)
  {
    pcl::PointCloud<pcl::PointXYZI> pl_save;
    for(const PointType &pt: *pl_orig)
    {
      pcl::PointXYZI ap;
      ap.x = pt.x; ap.y = pt.y; ap.z = pt.z;
      ap.intensity = pt.intensity; // Save intensity information to PCD file
      pl_save.push_back(ap);
    }
    string pcdname = savename + "/original_res_" + to_string(count) + ".pcd";
    pcl::io::savePCDFileBinary(pcdname, pl_save); 
    printf("Original resolution PCD saved: %s (%zu points)\n", pcdname.c_str(), pl_save.size());
  }
      ...
      for(int i=0; i<bl_tem->size() && n.ok(); i++)
      {
        ...
        for(int j=0; j<win_size; j++)
        {
          ...
          for(pcl::PointXYZI pp: plbuf[j]->points)
          {
            ...
            pt.intensity = pp.intensity; // Copy intensity information from original point
            pl_lc.push_back(pt);
          }
        }
...
public:
  ...
  void align_gravity(vector<IMUST> &xs)
  {
    ...
    double g_norm = g0.norm();
    
    
    Eigen::Vector3d n0 = g0 / g_norm;
  ...
  void motion_blur(pcl::PointCloud<PointType> &pl, PVec &pvec, IMUST xc, IMUST xl, deque<sensor_msgs::Imu::Ptr> &imus, double pcl_beg_time, IMUST &extrin_para)
  {
    ...
    // Original loop that skips first and last IMU data
    ...
    // Vector to store dt and head.t values for logging
    vector<pair<double, double>> dt_head_pairs;
    
    auto it_pcl = pl.end() - 1;
    // Original forward iteration - caused incomplete motion compensation for negative timestamps
    // for(auto it_kp=imu_poses.begin(); it_kp!=imu_poses.end(); it_kp++)
    // {
    //   IMUST &head = *it_kp;
    
    // Use reverse iteration like ekf_imu.hpp for proper negative time handling
    //printf("MOTION_COMPENSATION: Starting compensation with %zu IMU poses\n", imu_poses.size());
    if(imu_poses.size() > 0) {
      //printf("MOTION_COMPENSATION: IMU time range [%.6f, %.6f]\n", imu_poses[0].t, imu_poses.back().t);
    }
    for(int i=imu_poses.size()-1; i>=0; i--)
    {
      IMUST &head = imu_poses[i];
      //printf("MOTION_COMPENSATION: Processing IMU[%d] time=%.6f\n", i, head.t);
      R_imu = head.R;
      acc_imu = head.ba;
      vel_imu = head.v;
      pos_imu = head.p;
      ...
      for(; it_pcl->curvature > head.t; it_pcl--)
      {
        ...
        //printf("MOTION_COMPENSATION: Compensating point time=%.6f, dt=%.6f\n", it_pcl->curvature, dt);
        
        // Store dt and head.t for logging
        dt_head_pairs.push_back(make_pair(dt, head.t));
    ...
    // Save motion compensation details
    save_motion_compensation_details(frame_counter, dt_head_pairs);
  }
  ...
  int motion_init(vector<pcl::PointCloud<PointType>::Ptr> &pl_origs, vector<deque<sensor_msgs::Imu::Ptr>> &vec_imus, vector<double> &beg_times, Eigen::MatrixXd *hess, LidarFactor &voxhess, vector<IMUST> &x_buf, unordered_map<VOXEL_LOC, OctoTree*> &surf_map, unordered_map<VOXEL_LOC, OctoTree*> &surf_map_slide, vector<PVecPtr> &pvec_buf, int win_size, vector<vector<SlideWindow*>> &sws, IMUST &x_curr, deque<IMU_PRE*> &imu_pre_buf, IMUST &extrin_para)
  {
    ...
    for(int iterCnt = 0; iterCnt < 10; iterCnt++)
    {
      ...
      for(int i=0; i<win_size; i++)
      {
        ...
        // Save timestamps during initialization
        frame_counter++;
        save_lidar_timestamps(pl_origs[i], frame_counter);
        save_imu_timestamps(vec_imus[i], frame_counter);
        ...
        if(converge_flag == 1)
        {
          ...
          pvec_update(pvec_buf[i], x_buf[i], pwld); // Original function - no performance overhead
        }
        else
        {
          for(pointVar &pv: *pvec_buf[i])
    ...
    for(pointVar &pv: *pvec_buf[i])
    {
      ...
      pt.intensity = pv.intensity; // Use actual sensor intensity data
...
public:
  ...
  vector<pcl::PointCloud<PointType>::Ptr> pl_orig_buf;  // Store original resolution point clouds
  ...
  VOXEL_SLAM(ros::NodeHandle &n)
  {
    ...
    n.param<bool>("General/use_full_resolution_odometry", use_full_resolution_odometry, false);
    n.param<bool>("General/is_save_original_resolution", is_save_original_resolution, false);
    
    // HDL32E timestamp offset parameters
    n.param<bool>("General/enable_hdl32e_timestamp_offset", feat.enable_hdl32e_timestamp_offset, false);
    n.param<double>("General/hdl32e_timestamp_offset", feat.hdl32e_timestamp_offset, 0.05);
    
    // HDL32E timing validation parameters
    n.param<bool>("General/is_valid_hdl32e_timing", feat.is_valid_hdl32e_timing, true);
    ...
    // Timestamp logging parameters
    n.param<bool>("Odometry/enable_timestamp_logging", enable_timestamp_logging, true);
    n.param<string>("Odometry/timestamp_log_dir", timestamp_log_dir, "/tmp/voxelslam_timestamps");
    
    // EKF timestamp logging parameters
    n.param<bool>("Odometry/enable_ekf_timestamp_logging", odom_ekf.enable_ekf_timestamp_logging, true);
    n.param<string>("Odometry/ekf_timestamp_log_dir", odom_ekf.ekf_timestamp_log_dir, "/media/yjcho/MT_009/data/voxel_slam/ekf_timestamp");
    
    // Raw LiDAR timestamp logging parameters
    n.param<bool>("Odometry/enable_raw_timestamp_logging", enable_raw_timestamp_logging, true);
    n.param<string>("Odometry/raw_timestamp_log_dir", raw_timestamp_log_dir, "/media/yjcho/MT_009/data/voxel_slam/raw_timestamp");
  ...
  {
    static int lio_call_count = 0;
    lio_call_count++;
    if(lio_call_count % 20 == 1) {
      printf("=== LIO STATE ESTIMATION DEBUG [%d] ===\n", lio_call_count);
      printf("pptr size: %d\n", pptr ? (int)pptr->size() : 0);
    }
    ...
    if(lio_call_count % 20 == 1) {
      printf("About to compute eigenvalues, match_num: %d\n", match_num);
      printf("nnt matrix:\n[%f %f %f]\n[%f %f %f]\n[%f %f %f]\n", 
             nnt(0,0), nnt(0,1), nnt(0,2),
             nnt(1,0), nnt(1,1), nnt(1,2),
             nnt(2,0), nnt(2,1), nnt(2,2));
    }
    ...
    for(int id: vec_idx)
    {
      ...
      if(keyframes->at(id)->exist)
      {
        ...
        for(int j=0; j<plsize; j++)
        {
          ...
          pv.pnt << ap.x, ap.y, ap.z;
          pv.intensity = ap.intensity; // Copy intensity from keyframe
          pv.pnt = xx.R * pv.pnt + xx.p;
          pvec.push_back(pv);
        }
  ...
  int initialization(deque<sensor_msgs::Imu::Ptr> &imus, Eigen::MatrixXd &hess, LidarFactor &voxhess, PLV(3) &pwld, pcl::PointCloud<PointType>::Ptr pcl_curr)
  {
    ...
    pwld.clear();
    vector<float> intensity_buf; intensity_buf.clear();
    pvec_update(pptr, x_curr, pwld, intensity_buf);

    win_count++;
    x_buf.push_back(x_curr);
    pvec_buf.push_back(pptr);
    ResultOutput::instance().pub_localtraj(pwld, intensity_buf, 0, x_curr, sessionNames.size()-1, pcl_path);
    ...
    std::cout << "pl_origs size : " << pl_origs.size() << std::endl;
  ...
  void thd_odometry_localmapping(ros::NodeHandle &n)
  {
    ...
    while(n.ok())
    {
      ...
      // MOTION_COMPENSATION_DEBUG: IMU 데이터와 포인트클라우드 동기화 추적
      size_t pcl_size_before = pcl_curr ? pcl_curr->size() : 0;
      size_t imu_size_before = imus.size();

      // std::cout << "pcl_size_before : " << pcl_size_before << std::endl;
      ...
      std::cout << "pcl_size_after : " << pcl_curr->size() << std::endl;
      ...
      else
      {
        ...
        // Save timestamps for current frame
        frame_counter++;
        save_lidar_timestamps(pcl_curr, frame_counter);
        save_imu_timestamps(imus, frame_counter);
        
        // Save motion compensation details from EKF
        save_motion_compensation_details(frame_counter, odom_ekf.motion_comp_data);

        // Store original resolution point cloud for later saving (before any filtering)
        if(is_save_original_resolution) {
          // Get the truly original data from feat.original_pointclouds (FIFO)
          if(!feat.original_pointclouds.empty()) {
            pcl::PointCloud<PointType>::Ptr pl_orig_copy(new pcl::PointCloud<PointType>(*feat.original_pointclouds.front()));
            pl_orig_buf.push_back(pl_orig_copy);
            feat.original_pointclouds.pop_front(); // consume in order
          }
        }

        std::cout << "pcl_size_after 2 : " << pcl_curr->size() << std::endl;
        
        pcl::PointCloud<PointType> pl_down = *pcl_curr;
        if(!use_full_resolution_odometry) {
          down_sampling_voxel(pl_down, down_size);
        }

        std::cout << "pl down size : " << pl_down.size() << std::endl;

        if(!use_full_resolution_odometry && pl_down.size() < 500)
        {
          pl_down = *pcl_curr;
        ...
        PVecPtr pptr(new PVec);
        var_init(extrin_para, use_full_resolution_odometry ? *pcl_curr : pl_down, pptr, dept_err, beam_err);

        static int lio_attempt_count = 0;
        lio_attempt_count++;
        if(lio_attempt_count % 20 == 1) {
          printf("=== ABOUT TO CALL LIO_STATE_ESTIMATION [%d] ===\n", lio_attempt_count);
          printf("pptr size after var_init: %d\n", pptr ? (int)pptr->size() : 0);
          printf("degrade_cnt: %d (bound: %d)\n", degrade_cnt, degrade_bound);
        }

        bool lio_success = lio_state_estimation(pptr);
        
        if(lio_attempt_count % 20 == 1) {
          printf("LIO result: %s, degrade_cnt: %d -> %d\n", 
                 lio_success ? "SUCCESS" : "FAILED", 
                 degrade_cnt, 
                 lio_success ? (degrade_cnt > 0 ? degrade_cnt-1 : 0) : degrade_cnt+1);
        }
        ...
        vector<float> intensity_buf; intensity_buf.clear();
        pvec_update(pptr, x_curr, pwld, intensity_buf);
        ResultOutput::instance().pub_localtraj(pwld, intensity_buf, jour, x_curr, sessionNames.size()-1, pcl_path);
      ...
      if(win_count >= win_size)
      {
        ...
        // Save original resolution PCD files
        if(is_save_original_resolution)
        {
          for(int i=0; i<mgsize; i++)
          {
            int orig_idx = win_base + i;
            if(orig_idx < pl_orig_buf.size()) {
              printf("Saving original_res_%d.pcd with %zu points\n", orig_idx, pl_orig_buf[orig_idx]->size());
              FileReaderWriter::instance().save_original_resolution_pcd(pl_orig_buf[orig_idx], x_buf[i], orig_idx, savepath + bagname);
            } else {
              printf("Missing original cloud for idx %d (buffer size %zu)\n", orig_idx, pl_orig_buf.size());
            }
          }
        }
        ...
        for(int i=mgsize; i<win_count; i++)
        {
          ...
          // Also manage original resolution buffer
          if(is_save_original_resolution && i < pl_orig_buf.size())
          {
            pcl::PointCloud<PointType>::Ptr pl_orig_tem = pl_orig_buf[i-mgsize];
            pl_orig_buf[i-mgsize] = pl_orig_buf[i];
            pl_orig_buf[i] = pl_orig_tem;
          }
        }
        ...
        for(int i=win_count-mgsize; i<win_count; i++)
        {
          x_buf.pop_back();
          pvec_buf.pop_back();
          if(is_save_original_resolution && !pl_orig_buf.empty())
            pl_orig_buf.pop_back();
        ...
        if(search_result.first >= 0 && search_result.second > juds[id])
        {
          if(icp_normal(*(std_manager->plane_cloud_vec_.back()), *(std_managers[id]->plane_cloud_vec_[search_result.first]), loop_transform, icp_eigval))
          {
            ...
            else
            {
              ...
              {
                if(drift_p / jours[id] < ratio_drift)
                {
