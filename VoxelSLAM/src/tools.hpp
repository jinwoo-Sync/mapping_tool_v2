/**
 * @file tools.hpp
 * @brief Utility functions and data structures for VoxelSLAM
 * 
 * Contains common mathematical operations, coordinate transformations,
 * data structures, and utility functions used throughout the SLAM system.
 */
...
void down_sampling_voxel(pcl::PointCloud<PointType> &pl_feat, double voxel_size)
{
  ...
  for(PointType &p_c : pl_feat.points)
  {
    ...
    else
    {
      ...
      pp.intensity = (pp.intensity * pp.curvature + p_c.intensity) / (pp.curvature + 1); // Average intensity values
      pp.curvature += 1;
    }
  }
